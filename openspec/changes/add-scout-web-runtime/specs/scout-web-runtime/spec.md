...
- **THEN** `turn_failed` carries reason `web-timeout`, the owned runtime exits before that event, and the next prompt starts a fresh live session while historical records remain viewable
...
- **THEN** the owned runtime process group is terminated before `turn_failed` is appended, the session is recorded as lost, and the next prompt starts a fresh runtime and a fresh session
...
Normal follow-up turns MUST reuse the owned live session and preserve its history. After any failure, runtime death, or controller restart the next prompt MUST use a fresh session and report restart, while historical records remain in DSH. Failed prompts MUST NOT be automatically retried. A new delegation packet MUST restore authoritative context.

#### Scenario: follow-up retains live history
- **WHEN** a successful turn is followed by another prompt for the same key in the same controller/runtime
- **THEN** the session id and earlier conversation remain available

#### Scenario: failure starts fresh live context
- **WHEN** a turn fails or its owning runtime/controller restarts
- **THEN** the next prompt receives a fresh session id and a restart notice; old history is not deleted
